...
import { CRITERIA_CONFIG } from '@/config/useCaseConfig';
...
export default function CriteriaSelector({ onCriteriaSelected, onCriteriaLoaded, onError }: CriteriaSelectorProps) {
  ...
  useEffect(() => {
    ...
    if (CRITERIA_CONFIG.length > 0) {
      const firstCriteria = CRITERIA_CONFIG[0];
  ...
  const loadSheetNames = async () => {
    ...
    for (const criteria of CRITERIA_CONFIG) {
  ...
  const criteriaOptions = CRITERIA_CONFIG.map(criteria => ({
    id: criteria.id,
    name: criteria.name,
