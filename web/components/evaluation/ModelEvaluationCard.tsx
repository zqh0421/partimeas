...
const ScoreDisplay: React.FC<ScoreDisplayProps> = ({ score, maxScore, criterion }) => {
  ...
  return (
    <div className="flex items-center justify-between py-2">
      <span className="text-sm text-gray-700 capitalize flex-1">{criterion}</span>
      <div className="flex items-center space-x-2">
        <div className={`px-2 py-1 rounded text-xs font-medium ${getScoreColor(percentage)}`}>
          {score}/{maxScore}
        </div>
...
const ModelEvaluationCard: React.FC<ModelEvaluationCardProps> = ({ 
  ...
  const getModelDisplayInfo = (modelId: string, modelName?: string) => {
    const modelMap: { [key: string]: { displayName: string; color: string } } = {
      ...
      'claude-3-sonnet': { displayName: 'Claude 3 Sonnet', color: 'bg-purple-500' },
      'claude-3-haiku': { displayName: 'Claude 3 Haiku', color: 'bg-indigo-500' },
    };
    ...
    <div className="border border-slate-200 rounded-lg bg-white hover:shadow-md transition-shadow duration-200">
          ...
          <div className="flex items-center space-x-3">
            <div>
              <h4 className="font-semibold text-gray-900">
                {responseIndex !== undefined ? `For response ${responseIndex + 1}` : modelInfo.displayName}
              </h4>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <div className={`px-3 py-1 rounded-full text-sm font-medium bg-blue-200 text-blue-800`}>
      ...
      <div className="p-4">
        ...
        {modelOutput.rubricScores && Object.keys(modelOutput.rubricScores).length > 0 ? (
          ...
          <div className="text-sm text-gray-500 italic">No evaluation scores available</div>
        )}

        {/* Detailed Rubric Display */}
        {showRubricDetails && showDetailedRubric && (
          <div className="mt-4 p-3 bg-gray-50 rounded-lg">
            <h6 className="text-sm font-medium text-gray-800 mb-3">Rubric Structure</h6>
            <div className="space-y-3">
              {rubricStructure.responseComponents.map(component => (
                <div key={component.id}>
                  ...
                  >
                    <span className="font-medium text-sm text-gray-700">{component.name}</span>
                  ...
                  {expandedSections.has(component.id) && (
                    <div className="mt-2 pl-4 space-y-2">
                      {component.criteria.map(criterion => (
                        <div key={criterion.id} className="border-l-2 border-gray-200 pl-3">
                          <div className="font-medium text-xs text-gray-800">{criterion.name}</div>
                          ...
                          {criterion.subcriteria.map(subcriteria => (
                            <div key={subcriteria.id} className="ml-2 mb-2">
                              <div className="text-xs font-medium text-gray-700">{subcriteria.name}</div>
                              <div className="space-y-1 mt-1">
                                {subcriteria.scoreLevels.map(level => (
                                  <div key={level.score} className="flex items-start space-x-2 text-xs">
                                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-white font-bold ${
                                      level.score === 2 ? 'bg-green-500' : 
                                      level.score === 1 ? 'bg-yellow-500' : 'bg-red-500'
                                    }`}>
                                      {level.score}
                                    </span>
                                    <div className="flex-1">
                                      <div className="font-medium text-gray-700">{level.meaning}</div>
      ...
      <div className="p-4 border-t">
        ...
        <div className="text-sm text-gray-600 leading-relaxed">
          {modelOutput.feedback || 'No detailed feedback available for this response.'}
        </div>
      </div>
      ...
      {modelOutput.suggestions && modelOutput.suggestions.length > 0 && (
        <div className="p-4 border-t">
          <h5 className="font-medium text-gray-800 mb-2">Improvement Suggestions</h5>
