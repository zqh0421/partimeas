...
export default function RubricEvaluator({
  ...
  useEffect(() => {
    console.log(
      `RubricEvaluator useEffect: testCases.length=${testCases.length}, shouldStart=${shouldStart}`
    );
  ...
  const evaluateRubric = async () => {
    ...
    try {
      ...
      const testCasePromises = testCases.map(
        async (testCase, testCaseIndex) => {
          try {
            ...
            const rubricEffectiveness = assessRubricEffectiveness(
              scores,
              testCase
            );
            const refinementSuggestions = generateRefinementSuggestions(
              scores,
              testCase
            );
            const testCaseSpecificSuggestions =
              generateTestCaseSpecificSuggestions(testCase, scores);

            // Update progress atomically using ref to avoid race conditions
            completedCountRef.current++;
            const progress =
              (completedCountRef.current / totalEvaluations) * 100;
  ...
  const evaluateTestCase = (testCase: any) => {
    ...
    defaultCriteria.forEach((criteria) => {
      ...
      switch (criteria.id) {
        ...
        case "completeness":
          score =
            hasDetailedInput && hasDetailedContext
              ? 5
              : hasDetailedInput || hasDetailedContext
              ? 4
              : 3;
        ...
        case "creativity":
          ...
          const hasCreativeElements = creativeKeywords.some(
            (keyword) =>
  ...
  const assessRubricEffectiveness = (
    scores: { [key: string]: number },
    testCase: any
  ): "high" | "medium" | "low" => {
    const scoreVariance = Math.sqrt(
      Object.values(scores).reduce(
        (sum, score) => sum + Math.pow(score - 3, 2),
        0
      ) / Object.values(scores).length
    );
  ...
  const generateRefinementSuggestions = (
    ...
    if (lowScores.length > 0) {
      suggestions.push(
        `Consider adding more specific criteria for ${lowScores
    ...
    if (highScores.length === Object.keys(scores).length) {
      suggestions.push(
        "Rubric may be too lenient - consider adding more challenging criteria"
      );
    }

    if (
      Object.values(scores).every((score) => score === Object.values(scores)[0])
    ) {
      suggestions.push(
        "Rubric criteria may not be differentiated enough - consider making them more specific"
      );
    }

    return suggestions.length > 0
      ? suggestions
      : ["Rubric appears to be working well for this test case"];
  };

  const generateTestCaseSpecificSuggestions = (
    testCase: any,
    scores: { [key: string]: number }
  ) => {
    const suggestions: string[] = [];
    ...
    if (testCase.input.length < 50) {
      suggestions.push(
        "Consider adding criteria for input complexity or detail level"
      );
    }

    if (!testCase.context || testCase.context.length < 20) {
      suggestions.push(
        "Add more detailed context to improve evaluation accuracy"
      );
    }
  ...
  if (isEvaluating) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-600 mb-2">Analyzing evaluation results...</p>
