...
export default function UnifiedAnalysis(props: UnifiedAnalysisProps) {
  ...
  useEffect(() => {
    if (
      props.outcomesWithModelComparison.length > 0 &&
      currentPhase === "complete"
    ) {
      setAnalysisStep("complete");
    } else if (
      hasStartedEvaluation ||
      props.shouldStartEvaluation ||
      isGeneratingOutputs
    ) {
    ...
    props.outcomesWithModelComparison.length,
    hasStartedEvaluation,
    props.shouldStartEvaluation,
    isGeneratingOutputs,
    currentPhase,
    ...
    try {
      ...
      const outputPromises = props.testCases.map(async (testCase, index) => {
        try {
          ...
          console.log(
            `✅ Completed test case ${index + 1}/${props.testCases.length}:`,
            data
          );
          ...
          // Update progress
          props.onEvaluationProgress(
            index,
            ((index + 1) / props.testCases.length) * 50
          ); // 50% for generation
      ...
      for (let i = 0; i < props.testCases.length; i++) {
        ...
        if (result.status === "fulfilled") {
          ...
          if (apiResponse?.success) {
            ...
            const selectedModels: string[] = Array.isArray(
              apiResponse.selectedAssistantsModels
            )
              ? apiResponse.selectedAssistantsModels
              : [];
      ...
      (processedTestCases as any).__selectedOutputModelIds =
        selectedModelsPerTestCase[0] || [];
    } catch (error) {
      console.error("❌ Error during model output generation:", error);
      props.onEvaluationError(
      ...
      if (props.isRealEvaluation) {
        ...
        const evaluationResults = testCasesWithOutputs.map((testCase) => ({
          ...
          refinementSuggestions: [
            "Model outputs generated successfully (mock evaluation)",
          ],
        }));
  ...
  useEffect(() => {
    ...
    props.selectedUseCaseId,
    props.testCases.length,
    props.onMultiLevelSelectionChange,
  ]);
  ...
  const hasValidSelections = Boolean(
    props.selectedUseCaseId && props.testCases.length > 0
  );
    ...
    },
    {
      ...
      content: (
        <AnalysisStep
          ...
          sessionId={
            props.testCaseSessionIds?.get(props.selectedTestCaseIndex) || null
          }
        />
      ),
    },
