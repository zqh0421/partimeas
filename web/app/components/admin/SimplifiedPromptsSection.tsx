...
export function SimplifiedPromptsSection({
  ...
  useEffect(() => {
    if (prompts.length > 0) {
      ...
      if (
        lastPrompt.content === "" &&
        lastPrompt.name.startsWith("New") &&
        !editingId
      ) {
        ...
        setTimeout(() => {
          if (promptRefs.current[lastPrompt.id]) {
            ...
            setTimeout(() => {
              const nameInput = promptRefs.current[
                lastPrompt.id
              ]?.querySelector(
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      ...
      if (
        editingPrompt &&
        (!editingPrompt.content.trim() || !editingPrompt.name.trim())
      ) {
        ...
        renderItem={(prompt) => (
          <List.Item
            ...
            style={{
              padding: "20px 0",
              backgroundColor:
                newPromptId === prompt.id ? "#f6ffed" : "transparent",
                  ...
                  <Space direction="vertical" size={4}>
                    ...
                    <Space>
                      <Button
                        ...
                        onClick={() =>
                          setEditingId(
                            editingId === prompt.id ? null : prompt.id
                          )
                        }
                        size="middle"
                      >
