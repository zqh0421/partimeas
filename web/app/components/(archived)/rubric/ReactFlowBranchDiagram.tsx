...
import { HistoryEntry, VersionData } from "@/app/types";
...
const CustomNode = ({ data, selected }: CustomNodeProps) => {
  const getActionColor = (action: string) => {
    switch (action) {
      ...
      case "star":
        return "bg-gray-100 border-blue-500";
      case "unstared":
        return "bg-gray-100 border-blue-500";
      case "current":
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2 flex-shrink-0">
        <div className="flex items-center space-x-2">
          ...
          {data.action === "current" && (
            <span className="px-1 py-0.5 bg-indigo-500 text-white text-xs rounded">
              CURRENT
            </span>
          )}
          <span className="font-medium text-xs">
            {data.action === "current" ? "(unnamed)" : data.version || data.id}
          </span>
      ...
      <div className="text-xs space-y-1 flex-1 overflow-y-auto">
        ...
        <div>
          <span className="font-medium">By:</span> {data.modifier}
        </div>
        {data.field && (
          <div>
            <span className="font-medium">Field:</span> {data.field}
          </div>
        )}
        {data.changeType && (
          <div>
            <span className="font-medium">Type:</span>{" "}
            {data.changeType.replace("_", " ")}
          </div>
        )}
      </div>
    </div>
...
function ReactFlowBranchDiagramContent({
  ...
  const generateCommitHash = (entry: HistoryEntry, index: number) => {
    ...
    return `${timestamp.slice(-6)}${modifier}${index
      .toString()
      .padStart(2, "0")}`;
  };
  ...
  const createBranchStructureFromParentId = (history: HistoryEntry[]) => {
    ...
    const calculateLevels = (
      ...
      if (children.length === 0) {
        ...
        const childPositions = children.map(
          (child) => nodePositions.get(child.id)?.x || 0
        );
        const avgX =
      ...
      if (position) {
        ...
        const node: Node = {
          ...
          data: {
            ...
            differenceSummary:
              entry.differenceSummary ||
              "e.g. +1 Communication criteria, Modified Safety description, Changed Theory category...",
    ...
    history.forEach((entry) => {
      if (
        entry.parentId &&
        nodePositions.has(entry.parentId) &&
        nodePositions.has(entry.id)
      ) {
        const edge: Edge = {
    ...
    mergeNodes.forEach((mergeNode) => {
      ...
      const leafNodes = history.filter((entry) => {
        return (
          !history.some((child) => child.parentId === entry.id) &&
          entry.id !== mergeNode.id
        );
      });
    ...
    if (latestNode && nodePositions.has(latestNode.id)) {
      ...
      console.log(
        `Created current state edge: ${latestNode.id} -> current-state (latest node)`
      );
    }
  ...
  const generateTimelineLayout = useCallback(
    (history: HistoryEntry[]) => {
    ...
    [criteriaName, isMergeMode]
  ); // Add isMergeMode to dependencies
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log(
        "All node IDs:",
        layoutData.nodes.map((n) => n.id)
      );
      console.log(
        "All edge connections:",
        layoutData.edges.map((e) => `${e.source} -> ${e.target}`)
      );
  ...
  const handleMergeSelectedNodes = useCallback(() => {
    ...
    // 获取选中的节点数据
    const selectedNodeData = localHistory.filter((entry) =>
    ...
    const mergeNode: HistoryEntry = {
      ...
      } selected versions: ${selectedNodeData
        .map((n) => n.version || "unknown")
        .join(", ")}`,
      version: `v${
        Math.max(
          ...selectedNodeData.map((n) =>
            parseInt((n.version || "v1.0").slice(1))
          )
        ) + 1
      }.0`,
      changeType: "merge_versions",
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-7xl w-full mx-4 max-h-[95vh] overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-gray-900">
                Overall Evaluation Criteria Version History: {criteriaName}
              </h2>
              <p className="text-sm text-gray-600">
                Git-style version tracking for the complete evaluation framework
              </p>
            </div>
              ...
              {!isMergeMode ? (
                ...
                <div className="flex items-center space-x-2">
                  <button
                    ...
                    title={
                      selectedNodes.length >= 2
                        ? `Merge ${selectedNodes.length} selected nodes`
            ...
            >
              ...
              >
                <div className="text-xs text-gray-600">
                  {isLoading
                    ? "Loading..."
                    : `${nodes.length} versions • ${edges.length} connections`}
                </div>
              </Panel>
              <Panel
              ...
              >
                <div className="text-xs text-gray-500">
                  <div>
                    Esc: Close • R: Reset Layout • C: Center Node • Drag nodes
                    to reposition
                  </div>
                </div>
              </Panel>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center space-x-2">
                        <div
                          ...
                          style={{
                            background:
                              "repeating-linear-gradient(to right, #10B981 0px, #10B981 2px, transparent 2px, transparent 4px)",
                            height: "2px",
                          }}
                        ></div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-green-50 p-3 rounded border border-green-200">
                      ...
                      <div className="text-green-600 text-sm">
                        {focusedNode.summary
                          ? focusedNode.summary
                          : "AI will generate version summary..."}
                      </div>
                    </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <div className="flex items-center justify-between">
            <div className="text-xs text-gray-500">
              Showing {filteredHistory.length} of {localHistory.length} versions
          ...
          <div className="mt-2 text-xs text-gray-400">
            💡 Tip: Click nodes to view details in floating panel • All
            connections are solid with arrows • Only connections to Current are
            dashed • Drag nodes to adjust positions • 📋 AI Version Summary • 📝
            Changes
          </div>
        </div>
...
export default function ReactFlowBranchDiagram(
  props: ReactFlowBranchDiagramProps
) {
  return (
    <ReactFlowProvider>
