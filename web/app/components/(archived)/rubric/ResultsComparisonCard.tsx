...
import { RubricVersion } from "@/app/types";
...
export default function ResultsComparisonCard({
  ...
  const [selectedCriteriaVersion, setSelectedCriteriaVersion] =
    useState<string>("");
  const [showFilteredResults, setShowFilteredResults] = useState(false);

  // Comparison mode state
  const [comparisonMode, setComparisonMode] = useState<"models" | "criteria">(
    "models"
  );
    ...
    (): EvaluationResult[] => [
      {
        ...
        llmResponse:
          "Example LLM response - replace with real evaluation results",
        scores: {
          "1": 4, // Example score
          "2": 3, // Example score
        },
  ...
  const filteredResults = useMemo(() => {
    return mockEvaluationResults.filter((result) => {
      const matchesSearch =
        ...
        (result.modelName &&
          result.modelName.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      const matchesCriteriaVersion =
        selectedCriteriaVersion === "" ||
        (result.criteriaVersion &&
          result.criteriaVersion === selectedCriteriaVersion);

      return (
        matchesSearch &&
        matchesCriteria &&
        matchesModel &&
        matchesCriteriaVersion
      );
    });
  }, [
  ...
  const availableCriteria = useMemo(() => {
    ...
    mockEvaluationResults.forEach((result) => {
      Object.keys(result.scores).forEach((criterion) =>
  ...
  const summaryStats = useMemo(() => {
    ...
    const averageScore =
      totalEvaluations > 0
        ...
        : 0;
    const highScores = filteredResults.filter(
      (result) => result.overallScore >= 4
    ).length;
  ...
  const isCriteriaExpanded = (criteriaId: string) =>
    expandedCriteria.has(criteriaId);
                        ...
                        {expandedSections.directMode && (
                          <div className="p-4">
                            <p className="text-sm text-gray-600 mb-4">
                              [Start by selecting test cases and/or models and
                              then run the evaluation on a certain criteria
                              version.]
                            </p>
                            <p className="text-sm text-gray-600 mb-4">
                              Run analysis tools directly on selected test cases
                              ...
                              <div className="space-y-3">
                                <div className="flex space-x-2">
                                  ...
                                  <select
                                    ...
                                    onChange={(e) =>
                                      setSelectedCriteriaVersion(e.target.value)
                                    }
                                    className="px-3 py-2 border border-gray-300 rounded-md text-sm"
                                  >
                                    <option value="">All Versions</option>
                                    {availableCriteriaVersions.map(
                                      (version) => (
                                            ...
                                            <tbody>
                                              {Object.entries(
                                                ...
                                                <React.Fragment
                                                  key={criteriaId}
                                                >
                                                  <tr className="border-t border-gray-100 hover:bg-blue-50 transition-colors cursor-pointer group">
                                                    <td className="px-3 py-2 text-sm text-gray-900">
                                                      {(() => {
                                                        const criteriaItem =
                                                          currentVersion.rubricItems.find(
                                                            (item) =>
                                                              item.id ===
                                                              criteriaId
                                                          );
