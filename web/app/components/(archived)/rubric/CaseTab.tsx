...
import { TabNavigation, CaseHeader, TestCaseManager } from "./common";

import { parseInputContent, combineInputContent } from "@/app/utils/utils";
...
export default function CaseTab({
  ...
  const [case1SubTab, setCase1SubTab] = useState<"concerning" | "strengths">(
    "concerning"
  );
  ...
  const handleDeleteTestCase = (useCaseId: string, testIndex: number) => {
    ...
    if (useCaseIndex !== -1) {
      updatedUseCases[useCaseIndex] = {
        ...updatedUseCases[useCaseIndex],
        testCases: updatedUseCases[useCaseIndex].testCases.filter(
          (_, i) => i !== testIndex
        ),
      };
      setCurrentVersion((prev) => ({
  ...
  const handleUpdateTestCase = (
    ...
    const useCaseIndex = updatedUseCases.findIndex((uc) => uc.id === useCaseId);
    if (useCaseIndex !== -1) {
      const testCaseIndex = updatedUseCases[useCaseIndex].testCases.findIndex(
        (tc) => tc.id === testCaseId
      );
      if (testCaseIndex !== -1) {
  ...
  const renderCase1Content = () => {
    const caseData = {
      ...
      description:
        "For use case: Providing reflective questions (and explanations for why those questions may be helpful) that the worker could use to facilitate discussion in a future teacher meeting …. including questions that help reflect on the teacher's strengths and concerning behaviors. The goal here is to help the S123 worker work with the teacher to help the teacher reflect on their strengths and any concerning behaviors, so that they could collaboratively work together to understand how the teacher could best bring out their strengths.",
      testCasesCount: 9,
    };
    const useCase = (currentVersion.useCases || []).find(
      (uc) => uc.id === caseData.useCaseId
    );
      ...
      <div className="space-y-4">
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <TabNavigation
            ...
            onTabChange={(tabId) =>
              setCase1SubTab(tabId as "concerning" | "strengths")
            }
            className="mb-4"
          />
          ...
          <TestCaseManager
            ...
            onDeleteTestCase={(testIndex) => {
              const actualIndex =
                case1SubTab === "concerning" ? testIndex : testIndex + 5;
  ...
  const renderCase2Content = () => {
    const caseData = {
      ...
      name: "Providing reflective questions for teacher meetings",
      description:
        "Create reflective questions that help workers facilitate discussion in future teacher meetings, addressing disconnects between framing and understanding",
      testCasesCount: 3,
    };
    const useCase = (currentVersion.useCases || []).find(
      (uc) => uc.id === caseData.useCaseId
      ...
      <div className="space-y-4">
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <TestCaseManager
            ...
            onAddTestCase={() => handleAddTestCase(caseData.useCaseId)}
            onDeleteTestCase={(testIndex) =>
              handleDeleteTestCase(caseData.useCaseId, testIndex)
            }
            onUpdateTestCase={(testCaseId, field, value) =>
              handleUpdateTestCase(caseData.useCaseId, testCaseId, field, value)
            }
