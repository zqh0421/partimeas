...
export default function RubricComparison({
  ...
  const mockEvaluationResults = useMemo(
    (): EvaluationResult[] => [
      {
        id: "example-eval",
        rubricVersionId: "1",
  ...
  const getCriteriaDifferences = (
    version1: RubricVersion,
    version2: RubricVersion
  ) => {
    const differences: {
      added: RubricItem[];
      removed: RubricItem[];
    ...
    version2.rubricItems.forEach((item2) => {
      const exists = version1.rubricItems.find(
    ...
    version1.rubricItems.forEach((item1) => {
      const exists = version2.rubricItems.find(
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">
                Compare versions, results, and criteria changes
              </p>
            </div>
                  ...
                  {versions.map((version) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-600 mb-2">
                        {version.rubricItems.length} criteria •{" "}
                        {version.testCases.length} test cases
                      </p>
                ...
                <div className="space-y-6">
                  ...
                  {selectedVersionData.length === 2 && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      ...
                      {(() => {
                        const differences = getCriteriaDifferences(
                          selectedVersionData[0],
                          selectedVersionData[1]
                        );
                        return (
                          <div className="space-y-4">
                            {differences.added.length > 0 && (
                              <div>
                                <h5 className="font-medium text-green-700 mb-2">
                                  ✅ Added Criteria ({differences.added.length})
                                </h5>
                                <div className="space-y-2">
                              ...
                              <div>
                                ...
                                <div className="space-y-2">
                                  {differences.removed.map((item) => (
                                    <div
                                      key={item.id}
                                      className="bg-red-100 p-2 rounded text-sm"
                                    >
                                      <div className="font-medium">
                                        {item.criteria}
                                      </div>
                                  ...
                                  return (
                                    ...
                                    >
                                      <span className="text-sm">
                                        {criteriaItem
                                          ? `${criteriaItem.category} - ${criteriaItem.criteria}`
                                          : `Criteria ${criteriaId}`}
                                      </span>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      ...
                      {(() => {
                        const comparison = getScoreComparison(
                          selectedResultData[0],
                          selectedResultData[1]
                        );
                        return (
                          <div className="space-y-3">
                            ...
                            <div className="border-t pt-3">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center space-x-4">
                                  <span className="text-sm">
                                    {selectedResultData[0].overallScore} →{" "}
                                    {selectedResultData[1].overallScore}
                                  </span>
                                  <span
                                    className={`text-sm font-medium ${
                                      selectedResultData[1].overallScore >
            ...
            <div className="p-6">
              <div className="mb-6">
                ...
                <p className="text-gray-600 mb-4">
                  Track how criteria have evolved across different versions and
                  see the impact on evaluation results.
                </p>
              </div>
                    ...
                    {versions.map((version) => (
                      ...
                      >
                        ...
                        <div className="text-sm text-gray-600 mb-2">
                          {version.rubricItems.length} criteria across{" "}
                          {
                            new Set(
                              version.rubricItems.map((item) => item.category)
                            ).size
                          }{" "}
                          categories
                        </div>
                ...
                <div className="bg-white border rounded-lg p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Array.from(
                      new Set(
                        versions.flatMap((v) =>
                          v.rubricItems.map((item) => item.category)
                        )
                      )
                    ).map((category) => (
                      <div key={category} className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="space-y-2">
                          {versions.map((version) => {
                            const count = version.rubricItems.filter(
                ...
                <div className="bg-white border rounded-lg p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="space-y-2">
                        {versions.map((version) => {
                          const versionResults = allResults.filter(
                            (r) => r.rubricVersionId === version.id
                          );
                          const avgScore =
                            versionResults.length > 0
                              ? versionResults.reduce(
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <div className="flex items-center justify-between">
            <div className="text-xs text-gray-500">
              ...
              {comparisonMode === "criteria" &&
                `Analyzing ${versions.length} versions`}
            </div>
