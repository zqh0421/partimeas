...
export default function ModelComparisonEvaluator({
  ...
  useEffect(() => {
    console.log(
      `ModelComparisonEvaluator useEffect: testCases.length=${testCases.length}, shouldStart=${shouldStart}`
    );
  ...
  const generateRefinementSuggestions = (
    ...
    const allSuggestions = modelOutputs.flatMap(
    ...
    if (allScores.length > 0) {
      ...
      if (averageScore < 3.5) {
        suggestions.push(
          "Consider providing more specific context or examples in future test cases"
        );
      }

      // Check for structured output format compliance
      const hasStructuredOutput = modelOutputs.some(
        (output) => output.output && output.output.includes("===== SECTION")
      );

      if (!hasStructuredOutput) {
        suggestions.push(
          "Ensure models follow the required structured output format with section headers"
        );
      }
    }
