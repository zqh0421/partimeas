...
import { UseCaseConfig } from "@/app/types";
import { USE_CASE_CONFIGS } from "@/app/config/useCases";
...
export default function UseCaseSelector({
    ...
    async (useCaseId: string) => {
      ...
      try {
        const response = await fetch(
          `/api/use-case-data?useCaseId=${useCaseId}`
        );
  ...
  const organizeDataHierarchically = (
    ...
    testCases.forEach((testCase, index) => {
      ...
      const scenarioCategory =
        testCase.scenarioCategory || testCase.useCase || "General";
    ...
    const sheetUseCaseDescription =
      firstWithUseCaseDesc?.useCaseDescription || useCaseDescription;
  ...
  const handleTestCasesSelect = (testCaseIds: string[]) => {
    ...
    if (currentUseCase && selectedScenarioCategory) {
      const allTestCases =
        currentUseCase.scenarioCategories[selectedScenarioCategory].testCases;
      const selectedCases = allTestCases.filter((tc) =>
  ...
  const scenarioCategories = currentUseCase
    ...
    : [];
  const currentScenarioCategory =
    currentUseCase && selectedScenarioCategory
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-600">
            Choose a set of test cases from a use case.
          </p>
        </div>
        <div className="p-6">
                  ...
                  <div>
                    <h4 className="font-medium text-gray-900">
                      {selectedUseCase === useCase.id && currentUseCase
                        ? `Use Case #${
                            currentUseCase.sheetUseCaseId || useCase.id
                          }: ${
                            currentUseCase.sheetUseCaseDescription ||
                            useCase.description
                          }`
                        ...
                        {scenarioCategories.map((categoryId) => {
                          ...
                          return (
                            <button
                              key={categoryId}
                              onClick={() =>
                                handleScenarioCategorySelect(categoryId)
                              }
                            ...
                            <div className="mt-3 pt-3 border-t border-gray-200">
                              ...
                              >
                                <span className="text-sm font-semibold text-gray-900">
                                  Test Cases Summary ({selectedTestCases.length}{" "}
                                  cases)
                                </span>
                                <svg
                                  className={`w-4 h-4 transform transition-transform ${
                                    isTestCasesSummaryOpen ? "rotate-180" : ""
                                  }`}
                                  fill="none"
                                  stroke="currentColor"
                                  viewBox="0 0 24 24"
