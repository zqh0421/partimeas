...
import { createPrettifiedMarkdown } from "@/app/utils/markdownUtils";
...
const UNIFIED_SECTION_STYLES = {
  ...
  listItem:
    "flex items-start mb-2 p-2 bg-gray-50 rounded border-l-2 border-gray-300",
...
const parseContent = (content: string): ParsedContent => {
  ...
  if (structuredSections.length > 1) {
    ...
    for (
      let i = 0;
      i < sectionHeaders.length && i < structuredSections.length - 1;
      i++
    ) {
...
const formatSectionContent = (sectionName: string, content: string) => {
  ...
  switch (true) {
    ...
    case cleanSectionName.includes("developmental analysis") ||
      cleanSectionName.includes("theoretical analysis"):
    ...
    case cleanSectionName.includes("resources") ||
      cleanSectionName.includes("next steps"):
...
const formatAnalysisSection = (content: string): string => {
  ...
  formatted = formatted.replace(
    /\*\*([^*]+)\*\*/g,
    `<span class="${UNIFIED_SECTION_STYLES.emphasis} ${UNIFIED_SECTION_STYLES.primary.highlight} px-1 rounded">$1</span>`
  );

  // Add structure for paragraphs
  const paragraphs = formatted.split(/\n\s*\n/);
  formatted = paragraphs
    .map((p) => (p.trim() ? `<div class="mb-4">${p}</div>` : ""))
    .join("");
      ...
      .map((insight, index) => {
        if (insight.trim()) {
          ...
          <div class="flex-shrink-0 w-6 h-6 ${
            UNIFIED_SECTION_STYLES.primary.highlight
          } ${
            UNIFIED_SECTION_STYLES.primary.text
          } rounded-full flex items-center justify-center text-sm font-bold mr-3 mt-0.5">${
            index + 1
          }</div>
          <div class="${
            UNIFIED_SECTION_STYLES.primary.text
...
const formatResourcesSection = (content: string): string => {
  ...
  formatted = formatted.replace(
    /^(.*disclaimer.*|.*educational purposes.*|.*not professional.*)$/gim,
...
const formatGeneralSection = (content: string): string => {
  ...
  formatted = formatted.replace(
    /\*\*([^*]+)\*\*/g,
    `<span class="${UNIFIED_SECTION_STYLES.emphasis}">$1</span>`
  );

  // Format italic text
  formatted = formatted.replace(
    /\*([^*]+)\*/g,
    `<span class="${UNIFIED_SECTION_STYLES.subtle}">$1</span>`
  );
  ...
  const paragraphs = formatted.split(/\n\s*\n/);
  formatted = paragraphs
    .map((p) => (p.trim() ? `<div class="mb-3">${p}</div>` : ""))
    .join("");
    ...
    if (hasStructuredContent) {
      ...
      Array.from(sectionNames).forEach((sectionName, index) => {
        const group: GroupItem[] = modelOutputs.map((mo, modelIndex) => {
          ...
          const section = parsedContent.structuredSections.find(
            (s) => s.header === sectionName
          );
          return {
            modelId: mo.modelId,
  ...
  return (
    <div className="mb-6">
      ...
      <div className="space-y-6">
        {groupedContent.map(
          ({ group, groupIndex, isStructuredSection, sectionName }) => (
            <div
              key={groupIndex}
              ...
              return (
                ...
                >
                  ...
                  <div className="mb-4">
                    ...
                    modelOutput.suggestions.length > 0 ? (
                      <ul className="space-y-1">
                        {modelOutput.suggestions.map(
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        onClick={() =>
                          toggleOutputExpansion(modelOutput.modelId)
                        }
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="bg-white border rounded-lg p-4 shadow-sm">
                            ...
                            <div
                              ...
                              dangerouslySetInnerHTML={{
                                __html: createPrettifiedMarkdown(
                                  modelOutput.output
                                ),
                              }}
                            />
                          ...
                          <div className="bg-gray-50 border rounded-lg">
                            <div
                              ...
                              onClick={() =>
                                toggleOriginalTextExpansion(modelOutput.modelId)
                              }
                            >
