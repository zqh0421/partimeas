import RubricEvaluator, {
  EvaluationResult,
} from "@/app/components/RubricEvaluator";
...
export default function RunStep({
  ...
  useEffect(() => {
    console.log(`📊 RunStep Debug - Props:`, {
      ...
      localTestCasesWithModelOutputsLength:
        localTestCasesWithModelOutputs.length,
      currentPhase,
      shouldStartEvaluation,
    });
  }, [
  ...
  const generateOutputsPhase = async () => {
    ...
    try {
      console.log(
        `🚀 Starting parallel generation for ${testCases.length} test cases...`
      );
      ...
      const testCasePromises = testCases.map(async (testCase, index) => {
        ...
        if (!data.success) {
          throw new Error(
            `Test case ${index + 1}: ${
              data.error || "Output generation failed"
            }`
          );
        }
        ...
        if (result.status === "fulfilled") {
          ...
          if (totalExpectedOutputs === 0 && testCaseResult.outputs.length > 0) {
            totalExpectedOutputs =
              testCases.length * testCaseResult.outputs.length;
            setTotalOutputsExpected(totalExpectedOutputs);
            console.log(
              `📊 Updated total expected outputs: ${totalExpectedOutputs} (${testCases.length} test cases × ${testCaseResult.outputs.length} models)`
            );
          }
        } else {
          errors.push(
      ...
      console.log(
        `🎉 Parallel generation completed! Generated ${allOutputs.length} total outputs from ${testCases.length} test cases`
      );
      ...
      const testCasesWithOutputs = createTestCasesWithModelOutputs(
        testCases,
        results
      );
      ...
      console.log(
        `📋 Created testCasesWithModelOutputs:`,
      ...
      onEvaluationError(
        error instanceof Error
          ? error.message
          : "Unknown error during parallel generation"
      );
    } finally {
  ...
  const evaluateOutputsPhase = async (
    ...
    try {
      ...
      const evaluationPromises = testCasesWithOutputs.map(
        async (testCase, index) => {
          if (!testCase.modelOutputs || testCase.modelOutputs.length === 0) {
            console.warn(
              `⚠️ No model outputs for test case ${
                index + 1
              }, skipping evaluation`
            );
          ...
          if (!data.success) {
            throw new Error(
              `Test case ${index + 1}: ${data.error || "Evaluation failed"}`
            );
          }

          console.log(
            `✅ Evaluated test case ${index + 1}/${
              testCasesWithOutputs.length
            }: ${data.evaluations?.length || 0} evaluations`
          );
          return {
            testCaseIndex: index,
      ...
      for (let i = 0; i < evaluationResults.length; i++) {
        ...
        if (
          ...
          result.value.evaluatedOutputs.length > 0
        ) {
          const { testCaseIndex, evaluatedOutputs } = result.value;

          // Update each model output with evaluation results
          const updatedModelOutputs = updatedTestCasesWithOutputs[
            testCaseIndex
          ].modelOutputs.map((output) => {
            const evaluation = evaluatedOutputs.find(
              (evaluationResult: any) =>
                evaluationResult.modelId === output.modelId
            );
            if (evaluation) {
              return {
                ...output,
          ...
          console.error(
            `❌ Failed to evaluate test case ${i + 1}:`,
          ...
          <p className="text-gray-600">
            {currentPhase === "generating"
              ? `Processing ${testCases.length} test cases × ${
                  totalOutputsExpected / testCases.length || "multiple"
                } models = ${
                  totalOutputsExpected || testCases.length * 3
                } total outputs`
            ...
            <div className="text-center py-8">
              <div className="mb-4">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {outputsCompleted} / {totalOutputsExpected} model outputs
                  generated
                </p>
              </div>
              <p className="text-gray-600">
