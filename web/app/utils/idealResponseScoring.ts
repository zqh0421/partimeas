import { IdealResponseScore, RubricStructure, Criteria, Subcriteria, CriteriaData } from '@/app/types';
import { saveIdealResponseScores, restoreIdealResponseScores } from './selectionCache';
...
export function getOrInitializeIdealResponseScores(
  ...
  if (cachedScores.length > 0) {
    console.log(`[IdealResponseScoring] Restored ${cachedScores.length} cached scores for: ${idealResponseId}`);
    return cachedScores;
  }

  // Initialize new scores if not cached
  const newScores = initializeIdealResponseScores(idealResponseId, rubricStructure);
  
  if (newScores.length > 0) {
    // Save to cache
    saveIdealResponseScores(idealResponseId, newScores);
    console.log(`[IdealResponseScoring] Initialized ${newScores.length} default scores for: ${idealResponseId}`);
  }
...
export function updateIdealResponseScore(
  ...
  // Get current scores
  let currentScores = getOrInitializeIdealResponseScores(idealResponseId, rubricStructure);
  ...
  const existingScoreIndex = currentScores.findIndex(
    score => score.criteriaId === criteriaId && score.subcriteriaId === subcriteriaId
  );
  ...
  console.log(`[IdealResponseScoring] Updated score for ${criteriaId}/${subcriteriaId} to ${newScore} (cached: ${updatedScores.length} scores)`);
...
export function resetIdealResponseScores(
  ...
  const defaultScores = initializeIdealResponseScores(idealResponseId, rubricStructure);
  
  // Save to cache
  saveIdealResponseScores(idealResponseId, defaultScores);
  
  console.log(`[IdealResponseScoring] Reset ${defaultScores.length} scores to defaults for: ${idealResponseId}`);
...
export function getExpectedScore(
  ...
  const scores = getOrInitializeIdealResponseScores(idealResponseId, rubricStructure);
  
  const score = scores.find(
    s => s.criteriaId === criteriaId && s.subcriteriaId === subcriteriaId
  );
  
  console.log(`[IdealResponseScoring] getExpectedScore for ${criteriaId}/${subcriteriaId}:`, {
    idealResponseId,
    foundScore: !!score,
...
export function getIdealResponseScoreStats(
  ...
  const totalExpectedScore = scores.reduce((sum, s) => sum + s.expectedScore, 0);
 ...
 */
export function createBasicRubricFromCriteria(criteria: CriteriaData[]): RubricStructure {
  const scoreLevels = [
    { score: 0, meaning: "Inadequate", examples: ["Does not meet requirements"] },
  ...
  const responseComponents = Object.entries(categorizedCriteria).map(([category, categoryItems]) => ({
