  ...
  try {
    ...
    for (const config of CRITERIA_CONFIGS) {
      try {
        ...
        console.error(
          `[criteria-data] Failed to load from ${config.name}:`,
          error
        );
    ...
    if (!validation.isValid) {
      ...
      if (
        ...
        validation.errors[0] === "No criterion versions found"
      ) {
        console.warn(
          "[criteria-data] No criterion versions found - this is normal for initial setup"
        );
        ...
        return NextResponse.json({
          ...
          validation: {
            isValid: true,
            errors: [],
            warnings: ["No criterion versions found yet"],
          },
          totalCriteria: 0,
        });
      }
