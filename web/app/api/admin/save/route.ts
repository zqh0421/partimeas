...
export async function POST(request: NextRequest) {
  try {
    ...
    if (deletedPrompts.length > 0) {
      ...
      try {
        for (const deletedPrompt of deletedPrompts) {
          ...
          if (deleted) {
            console.log(
              `✅ Deleted prompt ${deletedPrompt.id} (${deletedPrompt.type})`
            );
          } else {
            console.warn(
              `⚠️ Failed to delete prompt ${deletedPrompt.id}: Prompt not found`
            );
          }
        }
      } catch (deleteError) {
    ...
    console.log(
      `📝 Found ${systemPrompts.length} system prompts and ${evaluationPrompts.length} evaluation prompts`
    );
    ...
    if (models.length > 0) {
      ...
      const enabledEvaluationModels = models.filter(
        (m) => m.isEnabled && m.isEvaluationModel
      );
      const enabledOutputModels = models.filter(
        (m) => m.isEnabled && m.isOutputGenerationModel
      );

      console.log(
        `🤖 Found ${enabledEvaluationModels.length} enabled evaluation models and ${enabledOutputModels.length} enabled output models`
      );
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        evaluationModels: models.filter(
          (m) => m.isEnabled && m.isEvaluationModel
        ).length,
        outputModels: models.filter(
          (m) => m.isEnabled && m.isOutputGenerationModel
        ).length,
        totalPrompts: prompts.length,
        systemPrompts: prompts.filter((p) => p.type === "system").length,
        evaluationPrompts: prompts.filter((p) => p.type === "evaluation")
          .length,
      },
    });
  } catch (error) {
