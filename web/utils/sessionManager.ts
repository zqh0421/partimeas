...
export async function getSessionStats(): Promise<{
  ...
  try {
    ...
    const sessionCountQuery =
      "SELECT COUNT(*) as count FROM partimeas_sessions";
    const responseCountQuery =
      "SELECT COUNT(*) as count FROM partimeas_responses";
    ...
    return {
      ...
      averageResponsesPerSession: parseFloat(
        avgResponsesResult[0]?.avg_count || "0"
      ),
