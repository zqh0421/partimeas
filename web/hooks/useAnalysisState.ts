...
import {
  ...
  RubricOutcomeWithModelComparison,
  IdealModelResponse,
} from "@/types";
...
export function useAnalysisState(): UseAnalysisStateReturn {
  ...
  const updateEvaluationState = useCallback(
    (updates: Partial<EvaluationState>) => {
  ...
  // UI setters
  const setUICurrentStep = useCallback(
    (step: AnalysisStep) => updateUIState({ currentStep: step }),
    [updateUIState]
  );
  const setUIIsLoading = useCallback(
    (isLoading: boolean) => updateUIState({ isLoading }),
    [updateUIState]
  );
  const setUIIsTestCasesSummaryOpen = useCallback(
    (isOpen: boolean) => updateUIState({ isTestCasesSummaryOpen: isOpen }),
    [updateUIState]
  );
  const setUIValidationError = useCallback(
    (error: string) => updateUIState({ validationError: error }),
    [updateUIState]
  );

  // Data setters
  const setDataTestCases = useCallback(
    (testCases: TestCase[]) => updateDataState({ testCases }),
    [updateDataState]
  );
  const setDataTestCasesWithModelOutputs = useCallback(
    (v: TestCaseWithModelOutputs[]) =>
      updateDataState({ testCasesWithModelOutputs: v }),
    [updateDataState]
  );
  const setDataCriteria = useCallback(
    (criteria: CriteriaData[]) => updateDataState({ criteria }),
    [updateDataState]
  );
  const setDataOutcomes = useCallback(
    (outcomes: RubricOutcome[]) => updateDataState({ outcomes }),
    [updateDataState]
  );
  const setDataOutcomesWithModelComparison = useCallback(
    (v: RubricOutcomeWithModelComparison[]) =>
      updateDataState({ outcomesWithModelComparison: v }),
    [updateDataState]
  );
  const setDataIdealResponses = useCallback(
    (idealResponses: IdealModelResponse[]) =>
      updateDataState({ idealResponses }),
    [updateDataState]
  );

  // Evaluation setters
  const setEvalShouldStart = useCallback(
    (shouldStart: boolean) =>
      updateEvaluationState({ shouldStartEvaluation: shouldStart }),
    [updateEvaluationState]
  );
  const setEvalProgress = useCallback(
    (progress: number) =>
      updateEvaluationState({ evaluationProgress: progress }),
    [updateEvaluationState]
  );
  const setEvalCurrentIndex = useCallback(
    (index: number) => updateEvaluationState({ currentTestCaseIndex: index }),
    [updateEvaluationState]
  );

  // Selection setters
  const setSelSelectedTestCaseIndex = useCallback(
    (index: number) => updateSelectionState({ selectedTestCaseIndex: index }),
    [updateSelectionState]
  );
  const setSelSelectedScenarioCategory = useCallback(
    (category: string) =>
      updateSelectionState({ selectedScenarioCategory: category }),
    [updateSelectionState]
  );
  const setSelSelectedCriteriaId = useCallback(
    (id: string) => updateSelectionState({ selectedCriteriaId: id }),
    [updateSelectionState]
  );
  const setSelSelectedCriteriaVersionId = useCallback(
    (id: string) => updateSelectionState({ selectedCriteriaVersionId: id }),
    [updateSelectionState]
  );
  const setSelSelectedIdealResponseId = useCallback(
    (id: string) => updateSelectionState({ selectedIdealResponseId: id }),
    [updateSelectionState]
  );
  const setSelSelectedSystemPrompt = useCallback(
    (prompt: string) => updateSelectionState({ selectedSystemPrompt: prompt }),
    [updateSelectionState]
  );

  const updateSystemPromptForUseCase = useCallback(
    (testCases: TestCase[]) => {},
    [setSelectionState]
  );
  ...
  const dataApi = useMemo(
    ...
    [
      ...
      updateDataState,
      setDataTestCases,
      setDataTestCasesWithModelOutputs,
      setDataCriteria,
      setDataOutcomes,
      setDataOutcomesWithModelComparison,
      setDataIdealResponses,
    ]
  );
  ...
  const selectionApi = useMemo(
    ...
    [
      ...
      setSelSelectedCriteriaVersionId,
      setSelSelectedIdealResponseId,
      setSelSelectedSystemPrompt,
    ]
  );
