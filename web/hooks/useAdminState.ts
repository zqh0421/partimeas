  ...
  const saveAssistantsOnly = async () => {
    try {
      ...
      for (const assistant of state.assistants) {
        ...
        const hasInvalidModels =
          !assistant.model_ids ||
          !Array.isArray(assistant.model_ids) ||
          assistant.model_ids.length === 0 ||
          !assistant.model_ids.every((id) => uuidRegex.test(id));
        const hasInvalidPrompt = !uuidRegex.test(
          assistant.system_prompt_id || ""
        );
        if (
          (isCreate || hasInvalidModels || hasInvalidPrompt) &&
          (hasInvalidModels || hasInvalidPrompt)
        ) {
          throw new Error(
            "Please select at least one saved Model and a System Prompt before saving assistants."
          );
        }
      }
      ...
      if (state.deletedAssistants && state.deletedAssistants.length > 0) {
        const deletePromises = state.deletedAssistants.map(
          async (deletedAssistant) => {
      ...
      if (state.hasConfigChanges) {
        const configPromises = state.configValues.map(async (config) => {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              `Failed to save config ${config.name}: ${
      ...
      setState((prev) => ({
        ...prev,
        error:
          error instanceof Error ? error.message : "Failed to save assistants",
      }));
    }
  };
  ...
  const removeModelConfig = (id: string) => {
    setState((prev) => {
      ...
      return {
        ...
        hasModelChanges: true,
        deletedModels:
          modelToRemove && modelToRemove.id.startsWith("temp-") === false
            ? [
                ...(prev.deletedModels || []),
                {
                  id,
                  provider: modelToRemove.provider,
                  model: modelToRemove.model,
                },
              ]
            : prev.deletedModels,
      };
  ...
  const removePromptConfig = (id: string) => {
    setState((prev) => {
      ...
      return {
        ...
        hasPromptChanges: true,
        deletedPrompts:
          promptToRemove && promptToRemove.id.startsWith("temp-") === false
            ? [
                ...(prev.deletedPrompts || []),
                { id, type: promptToRemove.type },
              ]
            : prev.deletedPrompts,
      };
    });
  ...
  const removeAssistant = (id: number) => {
    setState((prev) => {
      ...
      return {
        ...
        hasAssistantChanges: true,
        deletedAssistants:
          assistantToRemove && assistantToRemove.id > 0
            ? [
                ...(prev.deletedAssistants || []),
                { id, type: assistantToRemove.type },
              ]
            : prev.deletedAssistants,
      };
    });
