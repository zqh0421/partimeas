...
export function useSessionLoader(): UseSessionLoaderReturn {
  ...
  const [sessionData, setSessionData] = useState<SessionWithResponses | null>(
    null
  );
  ...
  try {
    ...
    console.warn(
      "useSearchParams or useRouter called outside of Suspense boundary:",
      error
    );
  ...
  const loadSession = useCallback(async (id: string): Promise<boolean> => {
    ...
    try {
      ...
      if (session.responses.length !== session.response_count) {
        throw new Error(
          `Session response count mismatch: expected ${session.response_count}, got ${session.responses.length}`
        );
      }
