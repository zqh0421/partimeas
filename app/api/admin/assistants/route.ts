...
export async function GET(request: NextRequest) {
  try {
    ...
    const assistants: Assistant[] = rows.map(row => ({
      id: row.id,
      name: row.name,
...
export async function POST(request: NextRequest) {
  try {
    const { name, model_id, system_prompt_id, required_to_show, type }: Partial<Assistant> = await request.json();

    if (!name || !model_id || !system_prompt_id || !type) {
      return NextResponse.json(
        { error: 'Missing required fields: name, model_id, system_prompt_id, type' },
        { status: 400 }
      );
    }
    ...
    if (!isValidUuid(model_id) || !isValidUuid(system_prompt_id)) {
      return NextResponse.json(
        { error: 'model_id and system_prompt_id must be valid UUIDs saved in the database' },
        { status: 400 }
      );
    }
    ...
    if (!modelExists) {
      return NextResponse.json(
        { error: 'Referenced model_id does not exist' },
        { status: 400 }
      );
    }
    ...
    const result = await sql`
      INSERT INTO partimeas_assistants (name, model_id, system_prompt_id, required_to_show, type)
      VALUES (${name}, ${model_id}, ${system_prompt_id}, ${required_to_show || false}, ${type})
      RETURNING id, name, model_id, system_prompt_id, required_to_show, type, created_at, updated_at
    `;
...
export async function PUT(request: NextRequest) {
  try {
    const { id, name, model_id, system_prompt_id, required_to_show, type }: Partial<Assistant> & { id: number } = await request.json();
    ...
    if (model_id !== undefined) {
      if (!isValidUuid(model_id)) {
        return NextResponse.json(
          { error: 'model_id must be a valid UUID' },
          { status: 400 }
        );
      }
      ...
      updateFields.model_id = model_id;
    }
    ...
    // Build dynamic update query using proper SQL construction
